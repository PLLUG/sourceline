  ...
  , mRemoteApiProvider{remoteApiProvider}
{
    ...
    connect(mProcess, SIGNAL(error(QProcess::ProcessError)), this, SLOT(reportError()), Qt::UniqueConnection);
}
...
void ClientProcess::start()
{
    ...
    qDebug() << "Running client:" << clientPath();
...
void ClientProcess::reportError()
{
    qDebug() << "ClientProcess::reportError:" << mProcess->errorString();
}
